...
private:
    ...
    void createImageViews() {
        ...
        for (uint32 i = 0; i < swapChainImages.size(); i++) {
            // TODO: Check why this uses mipLevel 1
            swapChainImageViews[i] = createImageView(swapChainImages[i], 1, swapChainImageFormat,
    ...
    void createGraphicsPipeline() {
        ...
        // Allows for an optional depth bounds test
    ...
    void createDepthResources() {
        ...
        transitionImageLayout(depthImage, 1, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    void createTextureImage() {
        ...
        // Largest dimension divided by 2, floor when isn't divisible and 1 so that original image has a level
        ...
        // generateMipmaps also transitions image to SHADER_READ_ONLY_OPTIMAL
        generateMipmaps(textureImage, textureImageFormat, texWidth, texHeight, mipLevels);
        vkDestroyBuffer(device, stagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);
    }

    void createTextureImageView() {
        textureImageView = createImageView(textureImage, mipLevels, textureImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
    }
    ...
    void loadModel() {
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                // 1.0f - because OBJ expects vertical coordinate of 0 being the bottom
                vertex.texCoord = {
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32 imageIndex) {
        ...
        // Order must be same as attachments
    ...
    void transitionImageLayout(VkImage image, uint32 mipLevels, VkFormat format, VkImageLayout oldLayout,
    ...
    // TODO: Make this happen once at startup and use the constant thereafter
    ...
    // TODO: revist this if for nothing else but better understanding
    // Also apparently not done at runtime so probably removed later
    // Will need function to load the mipmap data when we aren't generating it
    void generateMipmaps(VkImage image, VkFormat format, int32_t texWidth, int32_t texHeight, uint32 mipLevels) {
